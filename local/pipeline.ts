 ...
 */
import { fetchComtrade } from '../worker/agent/adapters/comtrade';
...
async function main() {
  ...
  for (const [i, entity] of targets.entries()) {
    ...
    try {
      const [comtrade, worldbank] = await Promise.all([
        fetchComtrade({ COMTRADE_API_KEY: cfg.comtradeKey } as never, entity.iso3!, years),
      ...
      const begun = await api.begin(entity.slug, runId);
      runId = begun.run_id;
  ...
  console.log(`\n${ok} ok, ${failed} failed, ${factsTotal} facts, ${mins} min`);
  if (errors.length) {
    console.log('\nFailures:');
